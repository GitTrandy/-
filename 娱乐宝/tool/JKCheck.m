...
+ (BOOL)validateMoblie:(NSString *)moblie{
    //手机号以13，15，18开头，八个\数字字符
    NSString *phoneRegex = @"^((13[0-9])|(15[^4,\\D])|(18[0,0-9]))\\d{8}$";
    NSPredicate *phoneText = [NSPredicate predicateWithFormat:@"SELF MATCHES%@",phoneRegex];
    return [phoneText evaluateWithObject:moblie];
}

//车牌号验证
+ (BOOL)validateCardNo:(NSString *)cardNo{
    NSString *carRegex = @"^[\u4e00-\u9fa5]{1}[a-zA-Z]{1}[a-zA-Z_0-9]{4}[a-zA-Z_0-9_\u4e00-\u9fa5]$";
    NSPredicate *carText = [NSPredicate predicateWithFormat:@"SELF MATCHES%@",carRegex];
    return [carText evaluateWithObject:cardNo];
...
+ (BOOL)validateCarType:(NSString *)CarType{
    ...
    NSPredicate *CarTypeText = [NSPredicate predicateWithFormat:@"SELF MATCHES%@",CarTypeRegex];
    return [CarTypeText evaluateWithObject:CarType];

}

//用户名
+ (BOOL)validateUserName:(NSString *)name{
    NSString *userNamrRegex = @"^[A-Za-z0-9]{6,20}+$";
    NSPredicate *userNamrTest = [NSPredicate predicateWithFormat:@"SELF MATCHES%@",userNamrRegex];
    return [userNamrTest evaluateWithObject:name];
...
+ (BOOL)validateIdentityCard:(NSString *)IdentityCard{
    ...
    NSPredicate *identityCardTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex2];
...
+ (BOOL)validateIDCardNumber:(NSString *)value
{
    ...
    switch (length) {
        case 15:
            ...
            if (year%4==0||(year%100==0&&year%4==0)) {//测试出生日期的合法性
          ...
          //  [reguluarExpression release];
            ...
            if (year%4==0||(year%100==0&&year%4==0)) {//测试出⽣生⽇日期的合法
                regularExpression = [[NSRegularExpression alloc]
